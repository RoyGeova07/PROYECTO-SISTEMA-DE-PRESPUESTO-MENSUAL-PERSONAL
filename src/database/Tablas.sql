...
--con esta restriccion se evita nombres duplicados de subcategoria por categoria(mejora de calidad de datos)
create unique INDEX ux_subcat_nombre_por_categoria on SUBCATEGORIA
...
Que es un indice?
Es una estructura (normalmente B-tree) que acelera bssquedas, joins, validaciones de FKs y 
...
(
    ...
    Id_alerta bigint generated by default as identity(start with 1,increment by 1)  primary key,
    Id_usuario bigint not null,
...
Ejemplo
Inicio: 2025/01, Fin: 2025/03 -> PASA (mismo año, 3 ≥ 1)
Inicio: 2025/10, Fin: 2026/01 -> PASA (2026 > 2025)
Inicio: 2025/05, Fin: 2025/04 -> FALLA (mismo año y 4 < 5)
Inicio: 2025/07, Fin: 2024/12 -> FALLA (2024 < 2025)

esto se evalua en cada insert y update. Si viola la condicion, la base de datos rechaza la fila con error 


*/
alter table PRESUPUESTO add constraint ck_presu_vigencia check((Anio_de_fin>Anio_de_inicio)or(Anio_de_fin=Anio_de_inicio and Mes_de_fin>=Mes_de_inicio));
